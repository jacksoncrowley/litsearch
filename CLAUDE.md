...
- `pubmed.py` — hits NCBI E-utilities (no API key needed). Defines the `Paper` dataclass. `search()` runs a **hardcoded broad PubMed query** (membrane/bilayer/protein-design/MD terms), fetches XML, parses into `Paper` objects. Client-side scoring does the fine-grained filtering — adding a new keyword group to the TOML does **not** automatically expand what PubMed returns.
